...
- **Never re-parent the product canvas between rows.** On narrow screens the canvas is mounted once inside `.catalogue__rail` and slid over the open row's `.row__stageSlot` by transform. Rendering it *inside* each row means React unmounts and remounts it on every tap, which tears down the WebGL context and rebuilds the renderer, the environment bake and every shader program — that was the "doesn't open smoothly".
- **Spin is accumulated with `delta`, never damped toward `clock.elapsedTime`.** A target derived from absolute elapsed time keeps advancing while the canvas is paused, so the first frame after any pause snaps to catch up. Every `useFrame` here also clamps `delta` to 0.05 so a stalled frame cannot arrive as one enormous step.
- **Do not give the in-card stage its own IntersectionObserver.** It lives inside a drawer with `overflow: hidden`; an element clipped to zero reports as not intersecting, which dropped `frameloop` to `'demand'` and stopped the rotation dead. Visibility is the parent's decision.
